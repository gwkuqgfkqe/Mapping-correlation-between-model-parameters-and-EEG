...
class TemporalAttentionModule(nn.Module):
    ...
    def forward(self, x):
        ...
        query = nn.Parameter(torch.randn(num_freqs, self.num_channels)).to(device)
        key = nn.Parameter(torch.randn(num_freqs, self.num_channels)).to(device)
        value = nn.Parameter(torch.randn(num_freqs, self.num_channels)).to(device)
...
class GraphTemporalConvolution(nn.Module):
    ...
    def forward(self, x):
        ...
        return x, self.importance  # .sum(dim=0)


class GraphTemporalConvolution1(nn.Module):
    def __init__(self, adjacency, in_channels, out_channels, kernel_length):
        super(GraphTemporalConvolution1, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.register_buffer('adjacency', adjacency)
        self.importance = nn.Parameter(torch.randn(in_channels, self.adjacency.size()[0], self.adjacency.size()[0]))
...
class GCNEEGNet(nn.Module):
    ...
    def __init__(self, n_channels, n_classes, input_window_size,
        ...
        self.con = nn.Conv2d(1, self.F1, (1, self.kernel_length),
                             stride=1, bias=False, padding='same')
        self.batch = nn.BatchNorm2d(self.F1, momentum=0.01, affine=True, eps=1e-3)
        self.graph = GraphTemporalConvolution(adjacency, self.F1, self.F1, 8)
        ...
        self.block_temporal_conv = nn.Sequential(
            nn.Conv2d(1, self.F1, (1, self.kernel_length),
                      stride=1, bias=False, padding='same'),
            nn.BatchNorm2d(self.F1, momentum=0.01, affine=True, eps=1e-3),
            GraphTemporalConvolution1(adjacency, self.F1, self.F1, 8),
            nn.BatchNorm2d(self.F1, momentum=0.01, affine=True, eps=1e-3),
            nn.ELU(),
    ...
    def forward(self, x):  # (64,22,1125)
        ...
        x, weights = self.frenquencyatten(x)
        # weights=0
...
class MergedModel(nn.Module):
    ...
    def forward(self, x):
        ...
        gcn_output, weights, weight_s = self.gcnEEGNet(x)  # (64,4)

        return gcn_output, weights, weight_s
